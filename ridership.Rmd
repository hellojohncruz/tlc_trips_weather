...
trip_weather_data <- 
  ...
  mutate(daily_trips = sum(total_trips),
         ...
         .keep = "none") |> 
  distinct() |> 
  mutate(type_of_day = case_when(max_heat_idx >= 90 ~ 'hot', 
                                 .default = 'normal'),
...
trip_weather_data |> 
  ggplot(aes(x = month, y = max_heat_idx, color = type_of_day)) +
  geom_jitter(stat = 'identity') +
  facet_grid(rows = vars(year(pickup_date))) +
  theme_classic() +
  labs(x = '', y ='', title = "Daily Heat Index", caption = "Figure 1") +
...
```{r week}
ggplot(aes(x = day_of_week, y = daily_trips), data = trip_weather_data) +
  geom_boxplot() +
  facet_grid(rows = vars(year(pickup_date))) +
  theme_bw() +
  labs(x = '', y = 'Total Trips')
```
...
```{r plot-heat-trips}
trip_weather_data |> 
  ggplot(aes(x = max_heat_idx, y = daily_trips, color = year)) + 
  geom_point(stat = 'identity') +
  labs(title = 'Heat Index vs. Total Trips', caption = "Figure 3", x = 'heat index', y = 'total trips') +
  theme_classic() +
  theme(legend.position = "top",
        legend.justification = c(0, 1)) +
  scale_color_manual(values = c('#4E79A7', '#F28E2B'), name = '')
...
```{r daily-precip}
ggplot(aes(x = daily_precip, y = daily_trips), data = trip_weather_data) +
  geom_point()
```
...
```{r model}
lm_mod <- lm(daily_trips ~ type_of_day + daily_precip + day_of_week + month + year, data = trip_weather_data)
summary(lm_mod)
```
