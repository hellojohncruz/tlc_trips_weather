...
dbExecute(cnxn, "CREATE VIEW trip_records AS 
                SELECT 
                  CASE 
...
# dbGetQuery(cnxn, "DESCRIBE trip_records")
```
...
query <- "SELECT 
          ...
          FROM trip_records
          WHERE 
            pickup_datetime > '2022-07-31 23:59:59' AND
            trip_time >= 0 AND 
            trip_time < 18000 AND 
            trip_miles >= 0 AND
            driver_pay > 0.01 AND
...
```{r data}
trip_weather_data <- 
  read_parquet("trip_weather_data.parquet")
```
...
all_trips <- 
  trip_weather_data |> 
  group_by(pickup_floor, heat_idx, total_precip) |> 
...
```{r heat-trips}
trip_weather_data |> 
  ggplot(aes(x = heat_idx, y = total_trips)) + 
  geom_point(stat = 'identity') +
...
seasonal <-
  trip_weather_data |> 
  mutate(day_of_week = as.factor(wday(pickup_floor, label = FALSE, week_start = 7)))

lm_mod <- lm(total_trips ~ app + heat_idx + total_precip + day_of_week, data = seasonal)
summary(lm_mod)
```